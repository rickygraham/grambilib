...
t_int *grambipan_tilde_order2(t_int *w)
{
    ...
    while (APn--)
    {
        ...
        if(sample2 != x->sample2_old || sample3 != x->sample3_old) { //use message audio input controls
                ...
                (*APout2++) =   sample1 * cosf(sample2); //X
                (*APout3++) =   sample1 * sinf(sample2); //Y
                (*APout4++) =   sample1 * (2 * cosf(sample2)); //U
                (*APout5++) =   sample1 * (2 * sinf(sample2)); //V
                } else {
                (*APout2++) =   sample1 * cosf(sample2) * cosf(sample3); //X
                (*APout3++) =   sample1 * sinf(sample2) * cosf(sample3); //Y
                (*APout4++) =   sample1 * (2 * cosf(sample2)) * cosf(sample3) * cosf(sample3); //U
                (*APout5++) =   sample1 * (2 * sinf(sample2)) * cosf(sample3) * cosf(sample3); //V
                }
                ...
                if(l == 0) { //solve multiply by zero issue
                    (*APout4++) =   sample1 * (2 * cosf(x->az_in));
                } else {
                    (*APout4++) =   sample1 * (2 * cosf(x->az_in)) * cosf(x->el_in) * cosf(x->el_in);;
                }
                
                if(l == 0) { //solve multiply by zero issue
                    (*APout5++) =   sample1 * (2 * sinf(x->az_in));
                } else {
                    (*APout5++) =   sample1 * (2 * sinf(x->az_in)) * cosf(x->el_in) * cosf(x->el_in);;
                }
...
t_int *grambipan_tilde_order3(t_int *w)
{
    ...
    while (APn--)
    {
        ...
        if(sample2 != x->sample2_old || sample3 != x->sample3_old) { //use message audio input controls
                ...
                (*APout4++) =   sample1 * (2 * cosf(sample2)); //U
                (*APout5++) =   sample1 * (2 * sinf(sample2)); //V
                (*APout6++) =   sample1 * (3 * cosf(sample2)); //P
                (*APout7++) =   sample1 * (3 * sinf(sample2)); //Q
                } else {
                (*APout2++) =   sample1 * cosf(sample2) * cosf(sample3); //X
                (*APout3++) =   sample1 * sinf(sample2) * cosf(sample3); //Y
                (*APout4++) =   sample1 * (2 * cosf(sample2)) * cosf(sample3) * cosf(sample3); //U
                (*APout5++) =   sample1 * (2 * sinf(sample2)) * cosf(sample3) * cosf(sample3); //V
                (*APout6++) =   sample1 * (3 * cosf(sample2)) * cosf(sample3) * cosf(sample3) * cosf(sample3); //P
                (*APout7++) =   sample1 * (3 * sinf(sample2)) * cosf(sample3) * cosf(sample3) * cosf(sample3); //Q
                }
                ...
                if(l == 0) { //solve multiply by zero issue
                    (*APout6++) =   sample1 * (3 * cosf(x->az_in));
                } else {
                    (*APout6++) =   sample1 * (3 * cosf(x->az_in)) * cosf(x->el_in) * cosf(x->el_in) * cosf(x->el_in);
                }
                
                if(l == 0) { //solve multiply by zero issue
                    (*APout7++) =   sample1 * (3 * sinf(x->az_in));
                } else {
                    (*APout7++) =   sample1 * (3 * sinf(x->az_in)) * cosf(x->el_in) * cosf(x->el_in) * cosf(x->el_in);
                }
